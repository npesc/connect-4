...
static char nom[20]; // nom du fichier de sauvegarde en variable générale
...
int verification(char grille[][7]) // fonction qui vérifie et determine le maximum de jetons alignés
{
    ...
    C.DX = 1;
    C.DO = 1;
    // COMPTEUR COLONNES 'X'(Zähler säulen X)
    ...
    // COMPTEUR COLONNES 'O' (Zähler säulen O)
    ...
    // Vérificateur lignes 'X' (Linienprüfer X)
    ...
    // Vérificateur lignes 'O' (Linienprüfer O)
    ...
    // Vérificateur diagonales / 'X' (Diagonalprüfer / X)
    ...
    // Vérificateur diagonales / 'O' (Diagonalprüfer / O)
